...
**Supersedes:** `@clack/core` + manual ANSI positioning trong wizard

---

## Context

**Phiên bản trước** của wizard dùng `@clack/core` primitives (`MultiSelectPrompt`, `SelectPrompt`, `TextPrompt`, `AutocompletePrompt`) với custom `render()` + alt-screen buffer + absolute-positioned ANSI footer. Approach đó gặp các giới hạn sau khi yêu cầu mở rộng:

1. **Split-pane layout**: left panel (~30% width) hiển thị breadcrumb / summary các step đã hoàn thành; right panel là widget active. `@clack/core` không có layout engine — muốn split-pane phải tự viết ANSI positioning engine.
...
5. **Focus chain** (tương lai): `@clack/core` không có.

**Triệu chứng cụ thể ở phiên bản trước**: `stepDetectTooling` dùng `@clack/prompts` inline log (`○ Tech stack: nextjs`, `● tsconfig.json`…) leak ra alt-screen canvas, đè lên footer absolute-positioned. Bug này là hệ quả của việc mix hai render model — không fix được triệt để nếu không thống nhất một framework.

## Decision

**Adopt [Ink 6](https://github.com/vadimdemedes/ink)** làm TUI framework duy nhất cho wizard. Tất cả step render qua Ink component tree. Giữ `@clack/prompts` **chỉ** cho spinner/intro/outro ngoài wizard alt-screen.

### Lý do

| Tiêu chí | Ink | @clack/core (giữ) | OpenTUI | blessed |
|---|---|---|---|---|
...
- **Giữ `@clack/core`** — fail trên yêu cầu split-pane + focus chain; phải viết layout engine riêng = reinventing Yoga.
- **OpenTUI** — Zig core cho perf tốt hơn Ink rõ rệt, nhưng Bun-first, Node support in-progress, tài liệu label "not production-ready". Rewrite hai lần (clack → OpenTUI) tệ hơn một lần (clack → Ink).
...
- **Rewrite toàn bộ wizard steps** từ clack primitive sang Ink component. Scope: `project-info.ts`, `tech-stack-select.ts`, `harness-config.ts`, `detect-tooling.ts`, `preview-apply.ts`, `bundle-select-screen.ts`, `text-prompt-screen.ts`.
- **Bundle tăng ~220 KB**. Chấp nhận được với CLI tool (không phải library).
- **Autocomplete downgrade nhẹ**: `ink-search-select` kém polished hơn `AutocompletePrompt` của clack. Hoặc compose `ink-text-input` + filtered list (~80 LoC).
...
- `@clack/prompts` inline prompts **không được dùng** bên trong wizard alt-screen (gây render conflict).
...
Tất cả step đã rewrite sang Ink component tree. `@clack/core` đã remove khỏi wizard deps. `@clack/prompts` giữ lại chỉ cho `commands/add.ts` (confirm re-install prompt ngoài wizard alt-screen). `CLAUDE.md` rule "Wizard = Ink only" đã bắt buộc.
