...
Installs the brainstorm-then-plan-then-code discipline — the habit of exploring the code, asking one clarifying question at a time, putting 2–3 approaches on the table with trade-offs, and waiting for explicit approval before touching the filesystem.

## What it installs

| Artifact | Path (in your project) | Purpose |
|----------|-----------------------|---------|
| Skill | `.agents/skills/planning-first/` | 5-phase protocol: explore → clarify → propose approaches → write plan → gate on approval |
| Rule | `.claude/rules/planning-first.md` | Always-loaded gate — no implementation code until the plan is approved; one clarifying question per message |
...
5. **Hard gate on approval.** No filesystem writes outside the plan doc until the user signs off. Applies to "simple" projects too.
