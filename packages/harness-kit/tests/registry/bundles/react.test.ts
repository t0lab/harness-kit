...
import { manifest } from '../../../src/registry/bundles/techstack/react/manifest.js'

const PACKAGE_ROOT = resolve(import.meta.dirname, '../../..')

describe('react bundle artifacts', () => {
  it('skill artifact references the upstream Vercel agent-skills repo', () => {
    const skill = manifest.common.artifacts.find(a => a.type === 'skill')
    expect(skill).toBeDefined()
    if (skill?.type !== 'skill') return
    expect(skill.src).toContain('github.com/vercel-labs/agent-skills')
    expect(skill.src).toContain('--skill vercel-react-best-practices')
  })
...
describe('react bundle install', () => {
  let dir: string

  beforeEach(async () => {
    dir = join(tmpdir(), `hk-react-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  it('copies rule and records bundle', async () => {
    await executeAdd(dir, 'react', { yes: true })

    const config = JSON.parse(await readFile(join(dir, 'harness.json'), 'utf-8'))
    expect(config.bundles).toContain('react')
    expect(existsSync(join(dir, '.claude/rules/react.md'))).toBe(true)
  }, 30000)
})
