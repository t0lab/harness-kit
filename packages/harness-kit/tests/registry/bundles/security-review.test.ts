...
import { manifest } from '../../../src/registry/bundles/workflow/security-review/manifest.js'

const PACKAGE_ROOT = resolve(import.meta.dirname, '../../..')

describe('security-review bundle artifacts', () => {
...
describe('security-review bundle install', () => {
  let dir: string

  beforeEach(async () => {
    dir = join(tmpdir(), `hk-security-review-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  it('copies skill + rule and records bundle', async () => {
    await executeAdd(dir, 'security-review', { yes: true })

    const config = JSON.parse(await readFile(join(dir, 'harness.json'), 'utf-8'))
    expect(config.bundles).toContain('security-review')
    expect(existsSync(join(dir, '.agents/skills/security-review/SKILL.md'))).toBe(true)
    expect(existsSync(join(dir, '.claude/rules/security-review.md'))).toBe(true)
  }, 15000)
})
