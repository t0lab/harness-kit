...
import { manifest } from '../../../src/registry/bundles/workflow/parallel-agents/manifest.js'

const PACKAGE_ROOT = resolve(import.meta.dirname, '../../..')

describe('parallel-agents bundle artifacts', () => {
...
describe('parallel-agents bundle install', () => {
  let dir: string
  beforeEach(async () => {
    dir = join(tmpdir(), `hk-parallel-agents-${Date.now()}-${Math.random().toString(36).slice(2)}`)
