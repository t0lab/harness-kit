...
import { manifest } from '../../../src/registry/bundles/techstack/fastify/manifest.js'

const PACKAGE_ROOT = resolve(import.meta.dirname, '../../..')

describe('fastify bundle artifacts', () => {
  it('skill artifact references the upstream mcollina/skills repo', () => {
    const skill = manifest.common.artifacts.find(a => a.type === 'skill')
    expect(skill).toBeDefined()
    if (skill?.type !== 'skill') return
    expect(skill.src).toContain('github.com/mcollina/skills')
    expect(skill.src).toContain('--skill fastify-best-practices')
  })
...
describe('fastify bundle install', () => {
  let dir: string

  beforeEach(async () => {
    dir = join(tmpdir(), `hk-fastify-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  it('copies rule and records bundle', async () => {
    await executeAdd(dir, 'fastify', { yes: true })

    const config = JSON.parse(await readFile(join(dir, 'harness.json'), 'utf-8'))
    expect(config.bundles).toContain('fastify')
    expect(existsSync(join(dir, '.claude/rules/fastify.md'))).toBe(true)
  }, 30000)
})
